...
from .core.generate_csv import load_pcap_to_model, get_urls
...
def reputation(request):
    ...
    API_KEY = 'a853196873bc2293ee70cfe30f24a4f3a1766109fa803900c1bbf5f949c85bfc'
    ...
    all_ips = analyze_dataframe(df).get_endpoints_ip()
    ...
    for ips in all_ips:
        ...
        url = socket.gethostbyaddr(ips)[0] 

        print(url)
        params = {'apikey': API_KEY, 'resource': url}
        response = requests.get(url, params=params)

        virustotal_json = response.json()
        context += virustotal_json['positives']
    '''




    return render(request, 'reputation.html', {'context': context})
